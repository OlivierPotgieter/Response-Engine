...
class ProductIdentifierConfig:
    ...
    TEXT_PROCESSING = {
        ...
        'stop_words': [
            ...
            'should', 'may', 'might', 'can', 'cant', 'wont', 'dont', 'isnt', 'arent'
        ],
        'product_keywords': [
            'gpu', 'graphics', 'card', 'cpu', 'processor', 'motherboard', 'ram',
            'memory', 'storage', 'ssd', 'hdd', 'monitor', 'display', 'keyboard',
            'mouse', 'headset', 'speaker', 'case', 'power', 'supply', 'cooler',
            'fan', 'rtx', 'gtx', 'radeon', 'ryzen', 'intel', 'amd', 'nvidia'
        ]
    }
    ...
    def validate_config(cls) -> Dict[str, Any]:
        ...
        if not cls.PINECONE_API_KEY:
            validation_results['errors'].append("PINECONE_API_KEY not found in environment")
            validation_results['valid'] = False
        ...
        if not cls.DB_CONFIG.get('password'):
            validation_results['warnings'].append("Database password not set - using empty password")
    ...
    def get_pinecone_config(cls) -> Dict[str, Any]:
        ...
        return {
            ...
            'metadata_config': {
                'indexed': [
                    'category', 'brand', 'brand_id', 'is_enabled',
                    'is_eol', 'price_range', 'product_type'
                ]
            }
        }
