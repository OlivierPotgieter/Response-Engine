...
class ProductIntelligenceBuilder:
    ...
    def build_complete_intelligence(self) -> Dict:
        ...
        """
        logger.info("🧠 Building complete product intelligence with manufacturer data...")
        ...
        all_manufacturers = self.db.get_all_manufacturers()  # Changed from get_all_brands
        categories = self.db.get_categories()
        manufacturer_names = self.db.get_manufacturer_names()  # Changed from get_brand_names

        # Filter out problematic products
        all_products = self._filter_quality_products(all_products_raw)

        logger.info(f"📊 Retrieved {len(all_products_raw)} total products, filtered to {len(all_products)} quality products")
        logger.info(f"📊 {len(all_manufacturers)} manufacturers, {len(categories)} categories")
        ...
        intelligence = {
            'metadata': {
                ...
                'total_manufacturers': len(all_manufacturers),  # Changed from total_brands
                'total_categories': len(categories)
            },
            'manufacturers': self._analyze_manufacturers(all_manufacturers, all_products),  # Changed from brands
    ...
    def _filter_quality_products(self, products: List[Product]) -> List[Product]:
        ...
        excluded_manufacturers = {'unknown', 'n/a', 'not specified', ''}  # Changed from brands
        excluded_categories = {'default', 'unknown', 'uncategorized', 'n/a', ''}

        for product in products:
            # Skip products with unknown/missing manufacturers
            if product.manufacturer_name.lower().strip() in excluded_manufacturers:  # Changed from brand_name
                continue
        ...
        logger.info(f"   Removed {len(products) - len(filtered_products)} low-quality products")

        return filtered_products

    def _analyze_manufacturers(self, manufacturers: List[Manufacturer], products: List[Product]) -> Dict:
        ...
        for manufacturer in manufacturers:
            ...
            manufacturer_categories = list(set(p.category for p in manufacturer_products if p.category))

            # Calculate average price for this manufacturer
            prices = [p.current_price for p in manufacturer_products if p.current_price > 0]
            ...
            manufacturer_analysis[manufacturer.name] = {
                ...
                'top_products': [
                    ...
                    for p in sorted(manufacturer_products, key=lambda x: x.popularity, reverse=True)[:5]
                ]
            }
    ...
    def _create_manufacturer_variations(self, manufacturer_name: str) -> List[str]:
        ...
        suffixes = [' inc', ' corp', ' ltd', ' limited', ' technology', ' tech', ' co', ' corporation']
        ...
        if len(words) > 1:
            acronym = ''.join(word[0].lower() for word in words if word and len(word) > 0)
            if len(acronym) >= 2:
    ...
    def _analyze_categories(self, categories: List[str], products: List[Product]) -> Dict:
        ...
        for category in categories:
            ...
            manufacturer_distribution = Counter(p.manufacturer_name for p in category_products)

            # Analyze price ranges
            prices = [p.current_price for p in category_products if p.current_price > 0]

            # Extract common terms from product names
            common_terms = self._extract_common_terms([p.name for p in category_products])

            # Extract model patterns
            model_patterns = self._extract_model_patterns([p.name for p in category_products])

            category_analysis[category] = {
                'name': category,
                'product_count': len(category_products),
                'manufacturer_distribution': dict(manufacturer_distribution.most_common(10)),  # Changed from brand_distribution
                'price_statistics': {
                    'min': min(prices) if prices else 0,
                    'max': max(prices) if prices else 0,
                ...
                'top_products': [
                    ...
                    for p in sorted(category_products, key=lambda x: x.popularity, reverse=True)[:10]
                ]
            }
    ...
    def _extract_search_patterns(self, products: List[Product]) -> Dict:
        ...
        patterns = {
            ...
            'memory_patterns': self._extract_memory_patterns(all_names + all_search_text),
            'storage_patterns': self._extract_storage_patterns(all_names + all_search_text),
    ...
    def _extract_tech_keywords(self, texts: List[str]) -> List[str]:
        ...
        tech_terms = [
            ...
            'ultrawide', '4k', '1440p', '144hz', '240hz', 'curved',
            'bluetooth', 'usb', 'hdmi', 'displayport', 'thunderbolt',
            'wifi', 'ethernet', 'gigabit', 'pcie', 'atx', 'mini-itx',
            'modular', 'gold', 'bronze', 'platinum', 'efficiency'
        ]
    ...
    def _calculate_statistics(self, products: List[Product]) -> Dict:
        ...
        manufacturer_dist = Counter(p.manufacturer_name for p in products if p.manufacturer_name)
        ...
        stats = {
            ...
            'manufacturer_distribution': dict(manufacturer_dist.most_common(20)),  # Changed from brand_distribution
    ...
    def save_intelligence(self, filepath: str = 'product_intelligence.json') -> bool:
        ...
        try:
            with open(filepath, 'w', encoding='utf-8') as f:
                json.dump(self.intelligence, f, indent=2, ensure_ascii=False, default=str)
    ...
    def get_summary(self) -> Dict:
        ...
        return {
            ...
            'total_manufacturers': metadata.get('total_manufacturers', 0),  # Changed from total_brands
            'total_categories': metadata.get('total_categories', 0),
            'filter_ratio': round((metadata.get('total_products_filtered', 0) / metadata.get('total_products_raw', 1)) * 100, 1),
            'price_range': {
            ...
            'top_manufacturers': list(stats.get('manufacturer_distribution', {}).keys())[:5],  # Changed from top_brands
...
def get_intelligence_summary() -> Dict:
    ...
    try:
        if builder.load_intelligence():
            ...
            return {'error': 'No intelligence file found. Run build_product_intelligence() first.'}
    finally:
        builder.close()
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"  Manufacturers: {summary.get('total_manufacturers', 0):,}")  # Changed from Brands
        print(f"  Categories: {summary.get('total_categories', 0):,}")
        print(f"  Price Range: ${summary.get('price_range', {}).get('min', 0):,.2f} - ${summary.get('price_range', {}).get('max', 0):,.2f}")
        print(f"  Average Price: ${summary.get('price_range', {}).get('average', 0):,.2f}")
