...
class IntentClassifier:
    ...
    def _load_model(self):
        ...
        try:
            ...
            logger.warning(f"Intent classification model not found at {self.model_path}")
    ...
    def predict_intent(self, comment: str) -> str:
        ...
        try:
            ...
            if self.pipeline is not None:
                ...
                logger.info(f"ML predicted intent '{intent_label}' for comment: {comment[:50]}...")
                return intent_label
            else:
    ...
    def _predict_intent_by_keywords(self, comment: str) -> str:
        ...
        warranty_keywords = ["warranty", "guarantee", "cover", "covered", "warrantee", "under warranty"]
        if any(keyword in comment_lower for keyword in warranty_keywords):
            logger.info(f"Keyword-based prediction: Warranty Inquiry for comment: {comment[:50]}...")
            return "Warranty Inquiry"
        
        pricing_keywords = ["price", "cost", "pricing", "how much", "what does", "r ", " r"]
        if any(keyword in comment_lower for keyword in pricing_keywords):
            return "Pricing Inquiry"
        
        stock_keywords = ["stock", "available", "availability", "in stock", "out of stock"]
        if any(keyword in comment_lower for keyword in stock_keywords):
            return "Stock Availability"
        
        logger.info(f"Keyword-based prediction: General Inquiry for comment: {comment[:50]}...")
        return "General Inquiry"
