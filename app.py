...
def log_request():
    ...
    logger.info(f"Request #{req_id}: {request.method} {request.path} from {request.remote_addr}")
...
def not_found(error):
    """Handle 404 errors"""
    return jsonify(build_api_error_response(
        "Endpoint not found",
        "not_found",
        404,
...
def root():
    ...
    return jsonify(response_builder.build_success_response(
        api_info,
        "Response Engine API is running"
    ))
...
def test_customer_request(request_id):
    ...
    try:
        ...
        processing_result = process_customer_request(request_id, generate_response=False)
        ...
        logger.info(f"Test request {request_id} completed with status: {response.get('status')}")
...
def process_customer_request_full(request_id):
    ...
    try:
        ...
        logger.info(f"Full processing request {request_id} completed with status: {response.get('status')}")
        return jsonify(response), status_code
...
def get_customer_request_summary(request_id):
    ...
    try:
        ...
        logger.info(f"Summary for {request_id} completed with status: {response.get('status')}")
        return jsonify(response), status_code
...
def debug_product_lookup(product_id):
    ...
    try:
        ...
        debug_data = {
            ...
            "lookup_successful": product_data is not None and not product_data.get('error'),
    ...
    try:
        ...
        if validation_result['status'] == 'success':
            validation_data = {
                ...
                "valid": True,
                "customer_comment_preview": validation_result.get('data', {}).get('customer_comment', '')[:100],
                "validation_timestamp": datetime.now().isoformat()
            }
            response = response_builder.build_success_response(validation_data, "Request ID is valid")
            status_code = 200
        else:
            response = response_builder.build_validation_error_response(validation_result)
            status_code = response.get('status_code', 400)
...
def get_stats():
    ...
    response_builder = get_response_builder()
    return jsonify(response_builder.build_success_response(stats_data, "Application statistics"))
...
if __name__ == '__main__':
    ...
    try:
        ...
        if pinecone_test.get('openai_connected') and pinecone_test.get('pinecone_connected'):
